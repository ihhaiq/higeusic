...
### تشغيل الفيديو من الخاص

يمكن لمالك البوت والمطورين المخوّلين إرسال الأمر في خاص البوت:

```text
فيديو @group_username رابط_الفيديو أو اسم_الفيديو
```

أو إرسال ملف فيديو ثم الرد عليه بـ `فيديو @group_username`. يعمل `/vplay` أيضاً، ويمكن استخدام آيدي مجموعة/قناة سالب معروف للحساب المساعد بدل اليوزر.

لا يحتاج البوت إلى عضوية في الوجهة. يجب إضافة أحد الحسابات المساعدة إليها مسبقاً، وفتح المكالمة والسماح للمساعد ببث الصوت والفيديو. لا يبدأ الأمر مكالمة جديدة ولا ينضم تلقائياً إلى مجموعة. عند تفعيل `PRIVATE_BOT_MODE` يجب تخويل الوجهة أيضاً.

تصل رسالة التشغيل وقائمة التحكم إلى الخاص، وتحمل الأزرار آيدي وجهة البث. لكل مجموعة قائمة انتظار وتحكم مستقلان حتى عند تشغيل عدة مجموعات من الخاص نفسه.
